...
#define STATION_ID "FELIPE_STATION_69"
#define PRINT_DATA_DELAY 3000
const String FIELD_SEPARATOR = ",";

/************ NETWORK SETTINGS ************/
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED }; //physical mac address
const byte ip[] = { 192, 168, 0, 40 }; // ip in lan
...
const byte agentip[] = { 192, 168, 0, 50 }; //ip on the agent.php was deployed
const int agentport = 80; //tcp port for it
EthernetClient agent; 
/******************************************/
...
void setup()
{
  ...
  Serial.println("Weather Shield online! " + String(STATION_ID));
...
void loop()
{
...

  {
    ...
    float currentSpeed;// = get_wind_speed(); ***** ESTA LINEA DA EL ERROR DE LECTURA DE VELOCIDAD DE VIENTO NAN (NOT A NUMBER)    
...
void printWeather()
{
  ...
  talkToAgent(getStationData());
 
  delay (PRINT_DATA_DELAY);

}

//get the station data. Produces ie: STATION_CHILE_01|135|0.5|57.69
String getStationData()
{
  String output;
 
  output = String(STATION_ID) + FIELD_SEPARATOR +
           String(winddir) + FIELD_SEPARATOR + 
           String(windspeedmph * 1.6, 1) + FIELD_SEPARATOR + 
           String(humidity);
...
/* Send the message to de Agent */
void talkToAgent(String message)
{
   Serial.println("Connecting to agent");
   if(agent.connect(agentip, agentport))
   {
